...
class iconshopAdminModel extends module
{
	...
	function getIconList($list_count = 20, $page_count = 10)
	{
		...
		if(in_array($search_limit->send_limit, array("Y", "N")))
		{
			$args->send_limit = $search_limit->send_limit;
		}
		if(in_array($search_limit->sell_limit, array("Y", "N")))
		{
			$args->sell_limit = $search_limit->sell_limit;
		}
		if(in_array($search_limit->event_limit, array("Y", "N")))
		{
			$args->event_limit = $search_limit->event_limit;
		}
		if(in_array($search_limit->minute_limit, array("Y", "N")))
		{
			$args->minute_limit = $search_limit->minute_limit;
		}
		if($search_keyword && in_array($search_target, $search_target_list))
		{
			if(array_search($search_target, $search_target_list) > 1)
			{
				$args->{$search_target} = (int)$search_keyword;
			}
			else
			{
	...
	function getLogList()
	{
		...
		if($s_regdate_less)
		{
			$args->s_regdate_less = (int)$s_regdate_less;
		}
		if($s_regdate_more)
		{
			$args->s_regdate_more = (int)$s_regdate_more;
		}
		if($search_keyword && in_array($search_target, $search_target_list))
		{
			if(array_search($search_target, $search_target_list) < 4)
			{
				$args->{$search_target} = (int)$search_keyword;
			}
			else
