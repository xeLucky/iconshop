...
class iconshopModel extends iconshop
{
	...
	function varSetting($data)
	{
        ...
        $tpl_url = sprintf('%s%s', \RX_BASEURL, "modules/iconshop/tpl/icons");
		if($data->event_limit == "Y")
		{
			$data->icon.= sprintf("<img src='%s/event.gif' title='event' alt='event' />&nbsp;",$tpl_url);
		}
		if($data->minute_limit == "Y")
		{
			$data->icon.= sprintf("<img src='%s/time.gif' title='time' alt='ime' />&nbsp;",$tpl_url);
		}
		if($data->regdate > $time_check)
		{
			$data->icon.= sprintf("<img src='%s/new.gif' title='new' alt='new' />&nbsp;", $tpl_url);
		}
		return $data;
	}
